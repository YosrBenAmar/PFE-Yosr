...
from tnfx_full_model.exporters import SHEET_ORDER, enrich_reporting_tables, export_tables
...
def test_output_manifest_has_run_id(config, tmp_path):
    ...
    assert "Hedge_Decision_Recommendations" in SHEET_ORDER
    assert "Gamma_R_Calibration_Global" in SHEET_ORDER


def test_output_manifest_preserved_across_two_pass_export(config, tmp_path):
    run_id = "20990101_020202"
    tables = {
        "Accepted_Profiles": pd.DataFrame([{"profile_id": 1, "family": "importer", "h_R": 0.1, "h_C": 0.2, "r": 0.1, "beta": 0.8, "fx_debt_service_share": 0.0, "target_prior_family_share": 0.35}]),
        "Inactive_Profiles": pd.DataFrame(columns=["profile_id", "family"]),
        "BM_Exposure_Diagnostics": pd.DataFrame([{"profile_id": 1, "family": "importer", "delta_CF_total": -0.08, "delta_profit_total": -0.8, "delta_net_EUR": -0.02, "delta_net_USD": -0.01, "currency_mismatch_gap": 0.0}]),
    ...
    tables = enrich_reporting_tables(tables, config)
    workbook_path = Path(tmp_path) / f"unit_{run_id}.xlsx"
    export_tables(
        tables,
        config,
        stage="full",
        run_id=run_id,
        output_dir_override=Path(tmp_path),
        workbook_name_override=workbook_path.name,
        write_csv=True,
        write_workbook=False,
    )
    assert "Output_Manifest" in tables
    first_manifest = tables["Output_Manifest"].copy()
    assert not first_manifest.empty
    export_tables(
        tables,
        config,
        stage="full",
    ...
    assert "Output_Manifest" in wb.sheetnames
    ws = wb["Output_Manifest"]
    assert ws.max_row > 1
    assert not tables["Output_Manifest"].empty
    required = {
        "run_id", "table_name", "file_name", "file_path", "n_rows", "n_columns",
        "export_status", "export_timestamp", "error_message",
    }
    assert required.issubset(set(tables["Output_Manifest"].columns))
